...
{
    ApplyNestedScrolling({m_typeMatchupList});
...
void ui::DatabasePanel::UpdateTypeMatchups()
{
    ...
    // 날씨 보정
    m_eventGuard([this, type] {
        std::unordered_map<pokegold::BattleWeather, wxRadioBox *> radioBoxes = {
            {pokegold::BattleWeather::Rain, m_typeWeatherModifierRainRadioBox},
            {pokegold::BattleWeather::Sun, m_typeWeatherModifierSunRadioBox},
            {pokegold::BattleWeather::Sandstorm, m_typeWeatherModifierSandstormRadioBox},
        };

        for (const auto &modifier : type.WeatherModifiers)
        {
            if (radioBoxes.contains(modifier.Weather))
            {
                auto *radioBox = radioBoxes[modifier.Weather];

                if (modifier.TypeEffectiveness == pokegold::TypeEffectiveness::MoreEffective)
                    radioBox->Select(1);
                else
                    radioBox->Select(2);

                radioBoxes.erase(modifier.Weather);
            }
        }

        for (const auto &[weather, radioBox] : radioBoxes)
            radioBox->Select(0);
    });
}
...
void ui::DatabasePanel::OnTypeWeatherModifierRadioBox(wxCommandEvent &event)
{
    event.Skip();
    ...
    {
        if (rain == 1)
            type.WeatherModifiers.emplace_back(pokegold::BattleWeather::Rain, pokegold::TypeEffectiveness::MoreEffective);
        else
            type.WeatherModifiers.emplace_back(pokegold::BattleWeather::Rain, pokegold::TypeEffectiveness::NotVeryEffective);
    }

    int sun = m_typeWeatherModifierSunRadioBox->GetSelection();
    if (sun > 0)
    {
        if (sun == 1)
            type.WeatherModifiers.emplace_back(pokegold::BattleWeather::Sun, pokegold::TypeEffectiveness::MoreEffective);
        else
            type.WeatherModifiers.emplace_back(pokegold::BattleWeather::Sun, pokegold::TypeEffectiveness::NotVeryEffective);
    }

    int sandstorm = m_typeWeatherModifierSandstormRadioBox->GetSelection();
    if (sandstorm > 0)
    {
        if (sandstorm == 1)
            type.WeatherModifiers.emplace_back(pokegold::BattleWeather::Sandstorm, pokegold::TypeEffectiveness::MoreEffective);
        else

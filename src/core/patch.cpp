...
std::vector<uint8_t> gs::core::patch::create_ips_patch(std::span<const uint8_t> original_bytes, std::span<const uint8_t> modified_bytes)
{
...
std::vector<uint8_t> gs::core::patch::create_xdelta_patch(std::span<const uint8_t> original_bytes, std::span<const uint8_t> modified_bytes)
{
	using namespace std;
	using namespace gs::utils;

	const auto &original_path = files::create_temp_file_path("xdelta_original.bin");
	filesystem::remove(original_path);
	files::write_bytes_to_file(original_path, original_bytes);

	const auto &modified_path = files::create_temp_file_path("xdelta_modified.bin");
	filesystem::remove(modified_path);
	files::write_bytes_to_file(modified_path, modified_bytes);

	const auto &xdelta_path = files::create_temp_file_path("xdelta_patch.xdelta");

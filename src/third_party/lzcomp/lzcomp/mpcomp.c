...
struct command *try_compress_multi_pass(const unsigned char *data, const unsigned char *flipped, unsigned short *size, unsigned flags)
{
    ...
    for (pos = (flags & 1); pos < *size; pos += (result[pos].count >= MULTIPASS_SKIP_THRESHOLD) ? result[pos].count : 1)
    {
        ...
        if ((result[pos].command >= 4) || (result[pos].count < MULTIPASS_SKIP_THRESHOLD))
            sources[current++] = pos;
    }
    free(reversed);
    free(sources);
    for (pos = 0; pos < *size; pos++)
    {
        for (current = 1; current < result[pos].count; current++)
            if (result[pos + current].count > result[pos].count)
            {
                result[pos].count = current;
                if ((result[pos].command == 2) && (current & 1) && !(flags & 2))
                    result[pos].count--;
            }
        if (result[pos].count <= command_size(result[pos]))
            result[pos] = (struct command){.command = 0, .count = 0};
    }
    for (pos = 0; pos < *size; pos++)
        if (!result[pos].command)
        {
            for (current = 1; (current < MAX_COMMAND_COUNT) && ((pos + current) < *size); current++)
                if (result[pos + current].command)
                    break;
                                     ...
                                     unsigned short length, unsigned short position, unsigned flags)
{
    ...
    if (result.count >= MULTIPASS_SKIP_THRESHOLD)
        return result;
    unsigned char p;
    for (p = 0; p < 3; p++)
    {
        ...
        if ((temp.command != 7) && (temp.count > result.count))
            result = temp;
    }
    if ((result.command >= 4) && (result.value >= (position - LOOKBACK_LIMIT)))
...
struct command pick_repetition_for_pass(const unsigned char *data, unsigned short length, unsigned short position, unsigned flags)
{
    ...
    if (data[position])
    {
        if ((position + 1) >= length)
            return (struct command){.command = 1, .count = 1, .value = data[position]};
        ...
        for (p = 1; ((position + p) < length) && (p < LOOKAHEAD_LIMIT); p++)
            if (data[position + p] != data[position + (p & 1)])
                break;
        result.count = p;
        return result;
    }
    ...
    {
        for (p = position + 1; (p < length) && (p < (position + LOOKAHEAD_LIMIT)); p++)
            if (data[p])
                break;
                                  ...
                                  unsigned short length, unsigned short position, unsigned flags)
{
    ...
    while (*sources >= 0)
    {
        refpos = *(sources++);
        if (command_type == 6)
            refpos = length - 1 - refpos;
        if (refpos >= (unsigned)(length - 3))
            current = buffer + refpos - (length - 3);
        else
            current = reference + refpos;
        if (memcmp(data + position, current, ((position + 4) > length) ? length - position : 4))
            continue;
        for (count = 4; (count < (unsigned)(length - position)) && (count < (length - refpos)); count++)
            if (data[position + count] != current[count])
                break;
        if (count > (length - refpos))
            count = length - refpos;
        if (count > (unsigned)(length - position))
